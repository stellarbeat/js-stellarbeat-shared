...
exports.default = {
    ...
    getClusterLeafNodes: function (cluster, map) {
        ...
        return this.getNodeChainRecursive(map.get(aNodeInCluster), map).filter(pk => {
    ...
    getCluster: function (node, map) {
        ...
        return this.getNodeChainRecursive(node, map).filter(pk => this.getNodeChainRecursive(map.get(pk), map).indexOf(node.publicKey) > -1);
    },
    ...
    getNodesWithQuorumSetsAndPublicKeys(nodes) {
        return lodash_1._.flow([this.getNodesWithPublicKeys, this.getNodesWithQuorumSets])(nodes);
    },
    getAllClusters(nodes, map) {
        let clusters = [];
        nodes.forEach(node => {
            let cluster = this.getCluster(node, map);
            if (clusters.every(alreadyDetectedCluster => !lodash_1._.isEqual(alreadyDetectedCluster, new Set(cluster)))) {
                clusters.push(new Set(cluster));
            }
        });
        return clusters;
    },
    getAllClusterLeafs(clusters, map) {
    ...
    hasQuorumIntersection(nodes, clusters, map) {
        ...
        let intersectingQuorums = (quorumsInLeaf.filter(quorum => quorumsInLeaf.every(otherQuorum => lodash_1._.intersection(otherQuorum, quorum).length > 0)));
        ...
        let otherClustersWithQuorums = clusters
            .filter(cluster => !lodash_1._.isEqual(cluster, clusterLeafs[0]))
    ...
    getQuorums: function (nodes) {
        return this
            ...
            .filter(possibleQuorum => this.isQuorum(possibleQuorum, this.getPublicKeyToNodeMap(nodes)));
    },
    ...
    getNodeChainRecursive: function (node, publicKeysToNodesMap, processedPublicKeys = []) {
        ...
        return quorum_set_1.QuorumSet.getAllValidators(node.quorumSet)
            ...
            .map(validator => publicKeysToNodesMap.get(validator))
            .reduce((processedPublicKeys, unProcessedNode) => this.getNodeChainRecursive(unProcessedNode, publicKeysToNodesMap, processedPublicKeys), processedPublicKeys);
    },
    ...
    getNodesThatContainASliceForEachMemberOfQuorum: function (quorum, publicKeyToNodesMap) {
        ...
        let nodes = quorum.filter(publicKey => publicKeyToNodesMap.get(publicKey)
            && this.hasSliceThatIsASubsetOfQuorum(publicKeyToNodesMap.get(publicKey).quorumSet, quorum));
    ...
    getPossibleQuorums: function (nodes) {
        return this.getAllCombinations(this.getNodesPublicKeysWithQuorumSets(nodes));
    },
    ...
    hasSubSetQuorumSliceOuter: function (quorumBitSet, quorumSet, cluster) {
        let validatorBitSets = quorumSet.validators.map(validator => this.getBitRepresentation(validator, cluster));
    ...
    getSlices: function (quorumSet) {
        ...
        let slices = this.getCombinationsOfSizeK(quorumSet.threshold, [].concat(quorumSet.validators).concat(quorumSet.innerQuorumSets));
    ...
    getCombinationsOfSizeK: function (k, nodesOrQSets) {
        ...
        for (let i = 0; i < nodesOrQSets.length; i++) {
            let prefixes = [];
            if (nodesOrQSets[i] instanceof quorum_set_1.QuorumSet) {
...
//# sourceMappingURL=quorum-service.js.map