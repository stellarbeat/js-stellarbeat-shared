...
class Network {
    constructor(nodes) {
        this._nodes = nodes;
        this._publicKeyToNodesMap = index_1.QuorumService.getPublicKeyToNodeMap(nodes);
    ...
    computeQuorumIntersection() {
        index_1.QuorumService.hasQuorumIntersection(this._nodes, this._clusters, this._publicKeyToNodesMap);
    }
    updateNetwork(nodes) {
        if (nodes) {
            this._nodes = nodes;
    ...
    detectClusters() {
        this._clusters = index_1.QuorumService.getAllClusters(this.nodes.filter(node => node.active && node.quorumSet.hasValidators()), this._publicKeyToNodesMap);
    ...
    getQuorumSetTomlConfig(quorumSet) {
        return index_1.generateTomlString(quorumSet, this._publicKeyToNodesMap);
    }
    createLinks() {
        this._links = _.flatten(this._nodes
            .filter(node => node.active && !this._failingNodes.includes(node))
            .map(node => {
            return index_1.QuorumSet.getAllValidators(node.quorumSet)
    ...
    createNodesForUnknownValidators() {
        this._nodes.forEach(node => {
            index_1.QuorumSet.getAllValidators(node.quorumSet).forEach(validator => {
                if (!this._publicKeyToNodesMap.has(validator)) {
                    let missingNode = new index_1.Node('unknown');
